...
def output(filename, sheet,num, name, present):
    my_file = Path('attend/attendance_files/'+filename+str(datetime.now().date())+'.xls');
    if my_file.is_file():
        rb = open_workbook('attend/attendance_files/'+filename+str(datetime.now().date())+'.xls');
        book = copy(rb);
        sh = book.get_sheet(0)
        # file exists
    else:
    ...
    book.save('attend/attendance_files/'+fullname)
    return fullname;
